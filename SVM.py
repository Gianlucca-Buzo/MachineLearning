...
svm = SVR(C=10,degree=2,gamma='scale',kernel='linear')
...
# Validação cruzada
cv_scores = cross_val_score(svm, X_train, y_train, cv=5, scoring='r2')
print(f'R² média da validação cruzada: {np.mean(cv_scores)}')

# Curvas de aprendizado
train_sizes, train_scores, test_scores = learning_curve(svm, X_train, y_train, cv=5, scoring='r2', n_jobs=-1, train_sizes=np.linspace(0.1, 1.0, 10))
...
### Script para decidir melhores hyperparametros
# param_grid = {
